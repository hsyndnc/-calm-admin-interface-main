...
const useCheckoutInfo = () => {
  return useQuery<{ shipName: string; shipAddress: string; shipCity: string; shipCountry: string }>({
    queryKey: ["checkout-info"],
    queryFn: async () => {
      const { data } = await apiClient.get("api/orders/checkout-info");
      return data;
    },
  });
};
...
const Checkout = () => {
  ...
  const { data: checkoutInfo } = useCheckoutInfo();
  ...
  useEffect(() => {
    if (checkoutInfo) {
      reset({
        shipName: checkoutInfo.shipName ?? "",
        shipAddress: checkoutInfo.shipAddress ?? "",
        shipCity: checkoutInfo.shipCity ?? "",
        shipCountry: checkoutInfo.shipCountry ?? "Turkey",
      });
    }
  }, [checkoutInfo, reset]);
  ...
  return (
    <CustomerLayout>
      <div className="max-w-3xl mx-auto px-4 py-6">
        <h1 className="text-xl font-semibold mb-6">Siparişi Tamamla</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-card border border-border rounded-xl p-6 flex flex-col">
            ...
            <Button form="checkout-form" type="submit" disabled={isPending} className="w-full">
